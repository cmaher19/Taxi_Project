...
yellowjan2016 <- left_join(yellowjan2016, taxi_zone, by=c('PULocationID'='LocationID')) %>%
  rename(pickup_borough = Borough, pickup_zone=Zone, pickup_servicezone=service_zone)

yellowjan2016 <- left_join(yellowjan2016, taxi_zone, by=c('DOLocationID'='LocationID')) %>%
...
mod1 <- lm(total_amount ~ trip_distance, data=yellowjan2016)
summary(mod1)
...
far_trips <- yellowjan2016 %>%
  filter(trip_distance > 18)

#MODEL: Using pickup and dropoff zone to predict trip distance
distance <- lm(trip_distance ~ pickup_zone + dropoff_zone, data=yellowjan2016)
summary(distance)
#plot(distance)

#predict on training set
yellowjan2016$total_prediction <- predict(mod1, yellowjan2016)
test_result <- yellowjan2016 %>%
...
yellowjan2016$factor_pickup = factor(x = yellowjan2016$pickup_borough,levels = c("Bronx", "Brooklyn", "Manhattan", "Staten Island", "Queens"))
yellowjan2016$factor_dropoff = factor(x = yellowjan2016$dropoff_borough,levels = c("Bronx", "Brooklyn", "Manhattan", "Staten Island", "Queens"))
yellowjan2016$trip_duration = as.numeric(yellowjan2016$trip_duration)

#density of trip_duration
duration_density <- yellowjan2016 %>%
...
mod2 <- lm(trip_duration ~ pickup_zone + dropoff_zone + pickup_hour + trip_distance, data=yellowjan2016)
summary(mod2)
...
ui <- shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput('pickup_month', 'Select Month', choices=as.character(unique(sort(yellowjan2016$pickup_month)))),
      selectInput('pickup_day', 'Select Date', choices=as.character(unique(sort(yellowjan2016$pickup_day)))),
      ...
      selectInput('pickup_hour', 'Select Hour of Pickup', 
                  choices=unique(sort(yellowjan2016$pickup_hour))), 
      selectInput('pickup_minute', 'Select Minute of Pickup', 
                  choices=as.character(unique(sort(yellowjan2016$pickup_minute)))),
      uiOutput('PBorough'),
...
server <- function(input, output) {
  output$PBorough <- renderUI(selectInput('pickup_borough',
                                          'Select Pickup Borough', c(unique(sort(yellowjan2016$pickup_borough))))
  )
  output$PNeighborhood <- renderUI(
    if(is.null(input$pickup_borough)) {return()}
                     ...
                     c(unique(sort(yellowjan2016$pickup_zone[which(yellowjan2016$pickup_borough == input$pickup_borough)])))
    ))
  
  output$DBorough <- renderUI(selectInput('dropoff_borough',
                                          'Select Dropoff Borough', c(unique(sort(yellowjan2016$dropoff_borough)))
  ))
  output$DNeighborhood <- renderUI(
    if(is.null(input$dropoff_borough)) {return()}
                     ...
                     c(unique(sort(yellowjan2016$dropoff_zone[which(yellowjan2016$dropoff_borough == input$dropoff_borough)])))
    ))
  
  sub1 <- reactive(
    yellowjan2016[which(yellowjan2016$pickup_borough==input$borough),]
  )
  sub2 <- reactive(
    sub1()[which(sub1()$pickup_zone == input$neighborhood),]
  )
  sub3 <- reactive(
    yellowjan2016[which(yellowjan2016$dropoff_borough==input$dborough),]
  )
  sub4 <- reactive(
  ...
  observeEvent(
    input$submit, {
      ...
      output$plot2 <- renderPlot(yellowjan2016 %>%
                                   filter(yellowjan2016$pickup_hour == pickup) %>%
      ...
      output$plot3 <- renderPlot(
        yellowjan2016 %>%
          filter(yellowjan2016$pickup_zone == input$pickup_zone) %>%
          ggplot(aes(x=total_amount)) + geom_density() 
        + annotate("segment", x=df.taxi$fare_prediction, xend=df.taxi$fare_prediction,
                   y=0, yend=0.1, color="Purple") 
      ...
      output$plot4 <- renderPlot(yellowjan2016 %>% 
      ...
      output$plot5 <- renderPlot(yellowjan2016 %>%
                                   filter(pickup_hour==input$pickup_hour) %>%
